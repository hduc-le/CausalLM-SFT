# forked and minor updated from https://github.com/nomic-ai/gpt4all/blob/main/gpt4all-training/train.py
import os
import torch
...
from transformers import AutoModelForCausalLM, AutoTokenizer, get_scheduler, DefaultDataCollator, set_seed
...
def formatting_func(example):
    text = f"### Question: {example['question_context']}\n ### Answer: {example['answer_text']}"
    return text
...
def train(accelerator, config):
    ...
    for epoch in range(0, config["num_epochs"]):
        train_loss = MeanMetric(nan_strategy="ignore").to(model.device)
        for step, batch in enumerate(pbar:=tqdm(train_dataloader, disable=not accelerator.is_main_process)):
